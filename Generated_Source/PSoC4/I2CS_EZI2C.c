...
volatile uint8 I2CS_curStatus; /* Status byte */
uint8 I2CS_fsmState;           /* FSM state   */

/* Variables intended to be used with Buffer 1: Primary slave address */
volatile uint8 * I2CS_dataBuffer1; /* Pointer to data buffer 1 */
uint16 I2CS_bufSizeBuf1;           /* Size of buffer 1 in bytes      */
uint16 I2CS_protectBuf1;           /* Start index of write protected area buffer 1 */
uint8 I2CS_offsetBuf1; /* Current offset within buffer 1 */
uint16 I2CS_indexBuf1;             /* Current index within buffer 1  */

#if(I2CS_SECONDARY_ADDRESS_ENABLE_CONST)
    uint8 I2CS_addrBuf1; /* Primary slave address. Used for software comparison   */
    uint8 I2CS_addrBuf2; /* Secondary slave address. Used for software comparison */

    /* Variables intended to be used with Buffer 2: Primary slave address */
    volatile uint8 * I2CS_dataBuffer2; /* Pointer to data buffer 2 */
    uint16 I2CS_bufSizeBuf2;           /* Size of buffer 2 in bytes */
    uint16 I2CS_protectBuf2;           /* Start index of write protected area buffer 2 */
    uint8 I2CS_offsetBuf2; /* Current offset within buffer 2 */
    uint16 I2CS_indexBuf2;             /* Current index within buffer 2 */
...
***************************************/
...
#if(I2CS_SECONDARY_ADDRESS_ENABLE_CONST)
    static uint32 I2CS_EzI2CUpdateRxMatchReg(uint32 addr1, uint32 addr2);
#endif /* (I2CS_SECONDARY_ADDRESS_ENABLE_CONST) */
    ...
    ***************************************/
    ...
    const I2CS_EZI2C_INIT_STRUCT I2CS_configEzI2C =
    {
        I2CS_EZI2C_CLOCK_STRETCHING,
        I2CS_EZI2C_MEDIAN_FILTER_ENABLE,
        I2CS_EZI2C_NUMBER_OF_ADDRESSES,
        I2CS_EZI2C_PRIMARY_SLAVE_ADDRESS,
        I2CS_EZI2C_SECONDARY_SLAVE_ADDRESS,
        I2CS_EZI2C_SUB_ADDRESS_SIZE,
        I2CS_EZI2C_WAKE_ENABLE,
        I2CS_EZI2C_BYTE_MODE_ENABLE
    };

    /*******************************************************************************
    * Function Name: I2CS_EzI2CInit
    ****************************************************************************//**
    *
    *  Configures the I2CS for EZI2C operation.
    *
    ...
    *  After initializing the I2CS in EZI2C mode using this function, 
    ...
    *******************************************************************************/
    void I2CS_EzI2CInit(const I2CS_EZI2C_INIT_STRUCT *config)
    {
        ...
        {
            /* Configure pins */
            I2CS_SetPins(I2CS_SCB_MODE_EZI2C, I2CS_DUMMY_PARAM,
                                                                      I2CS_DUMMY_PARAM);

            /* Store internal configuration */
            I2CS_scbMode       = (uint8) I2CS_SCB_MODE_EZI2C;
            I2CS_scbEnableWake = (uint8) config->enableWake;
            I2CS_scbEnableIntr = (uint8) I2CS_SCB_IRQ_INTERNAL;

            I2CS_numberOfAddr  = (uint8) config->numberOfAddresses;
            I2CS_subAddrSize   = (uint8) config->subAddrSize;
            ...
            I2CS_CTRL_REG     = I2CS_GET_CTRL_BYTE_MODE  (config->enableByteMode)    |
                                            I2CS_GET_CTRL_ADDR_ACCEPT(config->numberOfAddresses) |
                                            I2CS_GET_CTRL_EC_AM_MODE (config->enableWake);

            I2CS_I2C_CTRL_REG = I2CS_EZI2C_CTRL;

            /* Configure RX direction */
            I2CS_RX_CTRL_REG = I2CS_EZI2C_RX_CTRL |
                                        I2CS_GET_RX_CTRL_MEDIAN(I2CS_DIGITAL_FILTER_DISABLE);
                                                ;
            I2CS_RX_FIFO_CTRL_REG = I2CS_CLEAR_REG;

            /* Set default address and mask */
            if(I2CS_EZI2C_PRIMARY_ADDRESS == config->numberOfAddresses)
            {
                I2CS_RX_MATCH_REG = I2CS_EzI2CUpdateRxMatchReg(config->primarySlaveAddr,
            ...
            I2CS_TX_CTRL_REG      = I2CS_EZI2C_TX_CTRL;
            I2CS_TX_FIFO_CTRL_REG = ((0u != (config->enableClockStretch)) ?
                                                 I2CS_CLEAR_REG : I2CS_EZI2C_TX_FIFO_CTRL);
           ...
           I2CS_INTR_SPI_EC_MASK_REG = I2CS_NO_INTR_SOURCES;
            ...
            I2CS_INTR_SLAVE_MASK_REG  = I2CS_EZI2C_INTR_SLAVE_MASK;
            ...
            (void) CyIntSetVector(I2CS_ISR_NUMBER, (0u != (config->enableClockStretch)) ?
                                                                      (&I2CS_EZI2C_STRETCH_ISR) :
                                                                      (&I2CS_EZI2C_NO_STRETCH_ISR));

            if(0u != (config->enableClockStretch))
            {
                /* Configure interrupt sources */
                I2CS_INTR_SLAVE_MASK_REG |= I2CS_INTR_SLAVE_I2C_ADDR_MATCH;
                I2CS_INTR_RX_MASK_REG     = I2CS_NO_INTR_SOURCES;
            }
            else
            {
                /* Enable Auto ACK/NACK features */
                I2CS_I2C_CTRL_REG |= I2CS_EZI2C_CTRL_AUTO;

                /* Configure RX interrupt source */
                I2CS_INTR_SLAVE_MASK_REG |= I2CS_INTR_SLAVE_I2C_START;
                I2CS_INTR_RX_MASK_REG     = I2CS_INTR_RX_NOT_EMPTY;
            }
            ...
            I2CS_addrBuf1 = (uint8) config->primarySlaveAddr;
            I2CS_addrBuf2 = (uint8) config->secondarySlaveAddr;
        }
    }
    ...
    *
    *******************************************************************************/
    void I2CS_EzI2CInit(void)
    {
        /* Configure I2C interface */
        I2CS_CTRL_REG     = I2CS_EZI2C_DEFAULT_CTRL;
        I2CS_I2C_CTRL_REG = I2CS_EZI2C_DEFAULT_I2C_CTRL;
        ...
        /* Configure RX direction */
        I2CS_RX_CTRL_REG      = I2CS_EZI2C_DEFAULT_RX_CTRL;
        I2CS_RX_FIFO_CTRL_REG = I2CS_EZI2C_DEFAULT_RX_FIFO_CTRL;

        /* Set default address and mask */
        I2CS_RX_MATCH_REG     = I2CS_EZI2C_DEFAULT_RX_MATCH;

        /* Configure TX direction */
        I2CS_TX_CTRL_REG      = I2CS_EZI2C_DEFAULT_TX_CTRL;
        I2CS_TX_FIFO_CTRL_REG = I2CS_EZI2C_DEFAULT_TX_FIFO_CTRL;
        ...
        (void) CyIntSetVector(I2CS_ISR_NUMBER, &I2CS_EZI2C_STRETCH_ISR);
    #else
        (void) CyIntSetVector(I2CS_ISR_NUMBER, &I2CS_EZI2C_NO_STRETCH_ISR);
        ...
        /* Configure interrupt sources */
    #if (!I2CS_CY_SCBIP_V1)
        I2CS_INTR_SPI_EC_MASK_REG = I2CS_EZI2C_DEFAULT_INTR_SPI_EC_MASK;
    #endif /* (!I2CS_CY_SCBIP_V1) */

        I2CS_INTR_I2C_EC_MASK_REG = I2CS_EZI2C_DEFAULT_INTR_I2C_EC_MASK;
        I2CS_INTR_SLAVE_MASK_REG  = I2CS_EZI2C_DEFAULT_INTR_SLAVE_MASK;
        I2CS_INTR_MASTER_MASK_REG = I2CS_EZI2C_DEFAULT_INTR_MASTER_MASK;
        I2CS_INTR_RX_MASK_REG     = I2CS_EZI2C_DEFAULT_INTR_RX_MASK;
        I2CS_INTR_TX_MASK_REG     = I2CS_EZI2C_DEFAULT_INTR_TX_MASK;
        ...
        I2CS_addrBuf1 = I2CS_EZI2C_PRIMARY_SLAVE_ADDRESS;
        I2CS_addrBuf2 = I2CS_EZI2C_SECONDARY_SLAVE_ADDRESS;
    #endif /* (I2CS_SECONDARY_ADDRESS_ENABLE_CONST) */
    }
...
/*******************************************************************************
* Function Name: I2CS_EzI2CStop
****************************************************************************//**
*
*  Resets the EZI2C FSM into the default state.
*
*******************************************************************************/
void I2CS_EzI2CStop(void)
{
    I2CS_fsmState = I2CS_EZI2C_FSM_IDLE;
}
...
/*******************************************************************************
* Function Name: I2CS_EzI2CGetActivity
****************************************************************************//**
*
...
*  - I2CS_EZI2C_STATUS_READ1 - Read transfer complete. The transfer 
...
*  - I2CS_EZI2C_STATUS_WRITE1 - Write transfer complete. The buffer 
...
*  - I2CS_EZI2C_STATUS_READ2 - Read transfer complete. The transfer
*    used the secondary slave address. The error condition status bit must be 
*    checked to ensure that read transfer was completed successfully.
*  - I2CS_EZI2C_STATUS_WRITE2 - Write transfer complete. The buffer
*    content was modified. The transfer used the secondary slave address. 
*    The error condition status bit must be checked to ensure that write 
*    transfer was completed successfully.
*  - I2CS_EZI2C_STATUS_BUSY - A transfer intended for the primary 
...
*  - I2CS_EZI2C_STATUS_ERR - An error occurred during a transfer 
...
*    I2CS_EZI2C_STATUS_ERR and I2CS_EZI2C_STATUS_WRITE1/2 
*    is set. It is recommended to discard buffer content in this case.
*
* \globalvars
*  I2CS_curStatus - used to store the current status of the EZI2C 
*  slave.
*
*******************************************************************************/
uint32 I2CS_EzI2CGetActivity(void)
{
    ...
    status = I2CS_curStatus;

    /* Relay on address match event from HW as bus busy status */
    #if(I2CS_EZI2C_SCL_STRETCH_DISABLE)
    {
        /* For I2CS_CY_SCBIP_V0 the wake is prohibited by customizer */
        #if(I2CS_EZI2C_EC_AM_ENABLE)
        {
            status |= I2CS_CHECK_INTR_I2C_EC(I2CS_INTR_I2C_EC_WAKE_UP) ?
                        I2CS_EZI2C_STATUS_BUSY : 0u;
        }
        #else
        {
            status |= I2CS_CHECK_INTR_SLAVE(I2CS_INTR_SLAVE_I2C_ADDR_MATCH) ?
                        I2CS_EZI2C_STATUS_BUSY : 0u;
        }
        #endif
    }
    ...
    I2CS_curStatus &= ((uint8) ~I2CS_EZI2C_CLEAR_STATUS);
...
/*******************************************************************************
* Function Name: I2CS_EzI2CSetAddress1
****************************************************************************//**
*
...
*
* \globalvars
*  I2CS_addrBuf1 - used to store the primary 7-bit slave address 
*  value.
*
*******************************************************************************/
void I2CS_EzI2CSetAddress1(uint32 address)
{
    ...
    {
        I2CS_addrBuf1 = (uint8) address;

        I2CS_RX_MATCH_REG = 
                        I2CS_EzI2CUpdateRxMatchReg(address, (uint32) I2CS_addrBuf2);
    }
    #else
    {
        uint32 matchReg;
        ...
        /* Set address. */
        matchReg &= (uint32) ~I2CS_RX_MATCH_ADDR_MASK;
...
/*******************************************************************************
* Function Name: I2CS_EzI2CGetAddress1
****************************************************************************//**
*
...
uint32 I2CS_EzI2CGetAddress1(void)
{
    ...
    #if(I2CS_SECONDARY_ADDRESS_ENABLE)
    {
        address = (uint32) I2CS_addrBuf1;
    }
    #else
    {
        address = (I2CS_GET_RX_MATCH_ADDR(I2CS_RX_MATCH_REG) >>
...
/*******************************************************************************
* Function Name: I2CS_EzI2CSetBuffer1
****************************************************************************//**
*
...
*
* \globalvars
*  I2CS_dataBuffer1 – the pointer to the buffer to be exposed to the
*  master on a primary address.
*  I2CS_bufSizeBuf1 - the size of the buffer to be exposed to the 
*  master on a primary address.
*  I2CS_protectBuf1 - the start index of the read-only region in the
...
void I2CS_EzI2CSetBuffer1(uint32 bufSize, uint32 rwBoundary, volatile uint8 * buffer)
{
    if (NULL != buffer)
    {
        ...
        I2CS_dataBuffer1 =  buffer;
        I2CS_bufSizeBuf1 = (uint16) bufSize;
        I2CS_protectBuf1 = (uint16) rwBoundary;
...
/*******************************************************************************
* Function Name: I2CS_EzI2CSetReadBoundaryBuffer1
****************************************************************************//**
*
...
void I2CS_EzI2CSetReadBoundaryBuffer1(uint32 rwBoundary)
{
...
#if(I2CS_SECONDARY_ADDRESS_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: I2CS_EzI2CUpdateRxMatchReg
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    static uint32 I2CS_EzI2CUpdateRxMatchReg(uint32 addr1, uint32 addr2)
    {
        ...
        matchReg  = (uint32) (I2CS_GET_I2C_8BIT_ADDRESS(matchReg) << I2CS_RX_MATCH_MASK_POS);
        matchReg |= I2CS_GET_I2C_8BIT_ADDRESS(addr1);

        return(matchReg);
    }

    /*******************************************************************************
    * Function Name: I2CS_EzI2CSetAddress2
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  I2CS_addrBuf2 - used to store the secondary 7-bit slave address 
    *  value.
    *
    *******************************************************************************/
    void I2CS_EzI2CSetAddress2(uint32 address)
    {
        I2CS_addrBuf2 = (uint8) address;

        I2CS_RX_MATCH_REG = 
                        I2CS_EzI2CUpdateRxMatchReg((uint32) I2CS_addrBuf1, address);
    }


    /*******************************************************************************
    * Function Name: I2CS_EzI2CGetAddress2
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    ...
    {
        return((uint32) I2CS_addrBuf2);
    }


    /*******************************************************************************
    * Function Name: I2CS_EzI2CSetBuffer2
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  I2CS_dataBuffer2 – the pointer to the buffer to be exposed to the
    *  master on a secondary address.
    *  I2CS_bufSizeBuf2 - the size of the buffer to be exposed to the 
    *  master on a secondary address.
    *  I2CS_protectBuf2 - the start index of the read-only region in the
    ...
    *******************************************************************************/
    void I2CS_EzI2CSetBuffer2(uint32 bufSize, uint32 rwBoundary, volatile uint8 * buffer)
    {
        if (NULL != buffer)
        {
            ...
            I2CS_dataBuffer2 =  buffer;
            I2CS_bufSizeBuf2 = (uint16) bufSize;
            I2CS_protectBuf2 = (uint16) rwBoundary;
    ...
    /*******************************************************************************
    * Function Name: I2CS_EzI2CSetReadBoundaryBuffer2
    ****************************************************************************//**
    *
    *  Sets the read only boundary in the data buffer to be exposed to the I2C
...
#if(I2CS_EZI2C_WAKE_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: I2CS_EzI2CSaveConfig
    ****************************************************************************//**
    *
    ...
    *
    *******************************************************************************/
    void I2CS_EzI2CSaveConfig(void)
    {
    #if(I2CS_CY_SCBIP_V0)

        #if(I2CS_EZI2C_SCL_STRETCH_ENABLE)
        {
        ...
        enableInt = (uint8) I2CS_INTR_I2C_EC_WAKE_UP;
        ...
        #if(I2CS_EZI2C_SCL_STRETCH_ENABLE)
        {
            ...
            I2CS_SetI2CExtClkInterruptMode(enableInt);
        }
        #else
        {
            for(;;) /* Wait for end of transaction intended to slave */
            {
                if(0u == (I2CS_GetI2CExtClkInterruptSource() & I2CS_INTR_I2C_EC_WAKE_UP))
                {
            ...
            I2CS_SetSlaveInterruptMode(I2CS_NO_INTR_SOURCES);
    ...
    /*******************************************************************************
    * Function Name: I2CS_EzI2CRestoreConfig
    ****************************************************************************//**
    *
    ...
    *
    *******************************************************************************/
    void I2CS_EzI2CRestoreConfig(void)
    {
    #if(I2CS_CY_SCBIP_V0)
        ...
        #else
        {
            ...
            */
            ...
            I2CS_SetI2CExtClkInterruptMode(I2CS_NO_INTR_SOURCES);
            I2CS_SetSlaveInterruptMode(I2CS_EZI2C_INTR_SLAVE_MASK |
...
#endif /* (I2CS_EZI2C_WAKE_ENABLE_CONST) */
