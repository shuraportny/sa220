...
void ModbusUART_UartCyBtldrCommReset(void)
{
    ...
    ModbusUART_SpiUartClearRxBuffer();
    ModbusUART_SpiUartClearTxBuffer();
}
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        do
        {
            ...
            if (0u != ModbusUART_SpiUartGetRxBufferSize())
            {
                ...
                do
                {
                    byteCount = ModbusUART_SpiUartGetRxBufferSize();
                    CyDelayUs(ModbusUART_UART_BYTE_TO_BYTE);
                }
                while (byteCount != ModbusUART_SpiUartGetRxBufferSize());

                byteCount = ModbusUART_BYTES_TO_COPY(byteCount, size);
        ...
        {
            pData[i] = (uint8) ModbusUART_SpiUartReadRxData();
        }
    }
...
cystatus ModbusUART_UartCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        ModbusUART_SpiUartPutArray(pData, (uint32) size);
