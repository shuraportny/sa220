...
#include "DACout.h"
    ...
    #define DACout_SetP4PinDriveMode(shift, mode)  \
    do { \
        DACout_PC =   (DACout_PC & \
                                (uint32)(~(uint32)(DACout_DRIVE_MODE_IND_MASK << \
                                (DACout_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (DACout_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define DACout_SetP4PinDriveMode(shift, mode)  \
        do { \
            DACout_USBIO_CTRL_REG = (DACout_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(DACout_DRIVE_MODE_IND_MASK << \
                                    (DACout_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (DACout_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		DACout_SetP4PinDriveMode(DACout__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(DACout_DR & (uint8)(~DACout_MASK));
    drVal = (drVal | ((uint8)(value << DACout_SHIFT) & DACout_MASK));
    DACout_DR = (uint32)drVal;
}
...
{
    return (uint8)((DACout_PS & DACout_MASK) >> DACout_SHIFT);
}
...
{
    return (uint8)((DACout_DR & DACout_MASK) >> DACout_SHIFT);
}
...
void DACout_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  DACout_INTCFG & (uint32)(~(uint32)position);
    DACout_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(DACout_INTSTAT & DACout_MASK);
	DACout_INTSTAT = maskedStatus;
    return maskedStatus >> DACout_SHIFT;
}
