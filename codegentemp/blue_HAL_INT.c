...
CY_ISR(CyBLE_Uart_Interrupt)
{
    ...
    uint8 receivedData[blue_HAL_Uart_FIFO_SIZE] = {0u};

    /* Evaluate RX interrupt event */
    srcInterrupt = blue_HAL_Uart_GetRxInterruptSourceMasked();

    if(0u != (srcInterrupt & blue_HAL_Uart_INTR_RX_NOT_EMPTY))
    {
        if(0u != (srcInterrupt & blue_HAL_Uart_INTR_RX_PARITY_ERROR))
        {
            uartRxStatus |= CyBLE_INTR_RX_PARITY_ERROR;
        }
        if(0u != (srcInterrupt & blue_HAL_Uart_INTR_RX_FRAME_ERROR))
        {
            uartRxStatus |= CyBLE_INTR_RX_FRAME_ERROR;
        }
        if(0u != (srcInterrupt & blue_HAL_Uart_INTR_RX_OVERFLOW))
        {
            uartRxStatus |= CyBLE_INTR_RX_OVERFLOW;
        }
        if(uartRxStatus == CyBLE_INTR_RX_SUCCESS)
        {
            bufferLength = (uint8)blue_HAL_Uart_SpiUartGetRxBufferSize();
            for(srcCount = 0u; srcCount < bufferLength; srcCount++)
            {
                receivedData[srcCount] = (uint8)blue_HAL_Uart_SpiUartReadRxData();
            }
        }
        else
        {
            blue_HAL_Uart_SpiUartClearRxBuffer();
        }
        for(uartTxStatus = 0u; uartTxStatus < bufferLength; uartTxStatus++)
        {
            CyBLE_pf_handle_uart_rx_data(receivedData[uartTxStatus]);
        }
        blue_HAL_Uart_ClearRxInterruptSource(srcInterrupt);
    }
    else
    {
    ...
    srcInterrupt = blue_HAL_Uart_GetTxInterruptSourceMasked();

    /* Stack manager TX uart complete */
    if(0u != (srcInterrupt & blue_HAL_Uart_INTR_TX_UART_DONE))
    {
        if(0u != (srcInterrupt & blue_HAL_Uart_INTR_TX_OVERFLOW))
        {
            /*Stack manager TX uart error */
            uartTxStatus |= CyBLE_INTR_TX_OVERFLOW;
        }
        CyBLE_pf_handle_uart_tx_complt();
        blue_HAL_Uart_ClearTxInterruptSource(srcInterrupt);
    }
    else
    {
