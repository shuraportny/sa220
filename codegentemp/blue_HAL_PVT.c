...
void CyBLE_Uart_Start (void)
{
        ...
        /* Setup ISR */
        blue_uart_isr_StartEx(&CyBLE_Uart_Interrupt);
...
{
    #if (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART)
        blue_HAL_Uart_SpiUartPutArray(dataBuf, (uint32)length);
    #endif /* (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART) */
}
...
/* Interface to blue_bless_isr_Enable function */
void CyBleHal_EnableGlobalInterrupts(void)
{
    blue_bless_isr_Enable();
}

/* Interface to blue_bless_isr_Disable function */
...
void CyBle_HalInit(void)
{
    ...
    blue_bless_isr_SetVector(&CyBLE_Bless_Interrupt);
...
void CYBLE_BlessStart(void)
{
    ...
    blue_bless_isr_StartEx(&CyBLE_Bless_Interrupt);
}
